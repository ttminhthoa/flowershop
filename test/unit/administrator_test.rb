...
class AdministratorTest < ActiveSupport::TestCase
  ...
  def test_should_not_rehash_password
    ...
    assert_equal administrators(:quentin), Administrator.authenticate('quentin2', 'monkey')
  end

  def test_should_authenticate_administrator
    assert_equal administrators(:quentin), Administrator.authenticate('quentin', 'monkey')
  end
  ...
  def test_should_remember_me_for_one_week
    ...
    administrators(:quentin).remember_me_for 1.week
    after = 1.week.from_now.utc
    assert_not_nil administrators(:quentin).remember_token
    assert_not_nil administrators(:quentin).remember_token_expires_at
    assert administrators(:quentin).remember_token_expires_at.between?(before, after)
  end

  def test_should_remember_me_until_one_week
    time = 1.week.from_now.utc
    administrators(:quentin).remember_me_until time
    assert_not_nil administrators(:quentin).remember_token
    assert_not_nil administrators(:quentin).remember_token_expires_at
    assert_equal administrators(:quentin).remember_token_expires_at, time
  end
...
protected
  def create_administrator(options = {})
    record = Administrator.new({ :login => 'quire', :email => 'quire@example.com', :password => 'quire69', :password_confirmation => 'quire69' }.merge(options))
    record.save
    record
  end
end
